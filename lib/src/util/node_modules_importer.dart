...
/// An importer that resolves URLs starting with node modules prefixes by
/// searching in `node_modules` directories. Allowed prefixes:
/// - `~`
/// - `node_modules/`
///
...
class NodeModulesImporter extends Importer {
  ...
  ///
  /// If [url] starts with `~` or `node_modules`, this searches all
  /// `node_modules` directories, using a [FilesystemImporter] to canonicalize
  ...
  Uri? canonicalize(Uri url) {
    ...
    var allowedNodeModulesPrefixes = ['~', 'node_modules/'];
    for (var prefix in allowedNodeModulesPrefixes) {
      if (url.path.startsWith(prefix)) {
        url = url.replace(path: url.path.substring(prefix.length));
      }
    }
